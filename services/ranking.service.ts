...
export async function rankCandidates(
  ...
  const { data } = await apiClient.post<RankCandidatesResponse>('/rank-candidates', formData);
  return data;
}
...
export async function getRankingHistory(userId: string): Promise<GetHistoryResponse> {
  const { data } = await apiClient.get<GetHistoryResponse>(`/get-history?user_id=${userId}`);
  return data;
}
